...
int main(int argc, char *argv[])
{
    ...
    else if (command == "list_playlists")
    {
        ...
        {
            std::cout << colorText(p.name, 34) << " (Avg Rating: " << std::fixed << std::setprecision(2) << p.averageRating() << ")\n";
        }
    }
        ...
        for (const auto& p : playlists) {
            if (p.name == playlistName) {
                ...
                for (size_t i = 0; i < p.songs.size(); ++i) {
                    ...
                    std::cout << i << ": " << colorText(s.title, 34) << " by " << s.artist 
                    << " (" << colorText(formatDuration(s.duration), 35) << " mins)\n";                }
                if (p.songs.empty()) {
